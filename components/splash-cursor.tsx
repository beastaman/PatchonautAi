...
export function SplashCursor({
  ...
  useEffect(() => {
    ...
    if (!canvas) return;
  // Now canvas is non-null. Optionally, create a new variable:
  const safeCanvas = canvas;

    // Define pointer value directly (instead of a prototype function)
    ...
    // Get WebGL context (WebGL2 if available)
    ...
    const halfFloatTexType = 
        isWebGL2 && (gl as WebGL2RenderingContext).HALF_FLOAT ? 
    ...
    if (isWebGL2) {
        ...
        formatRG = getSupportedFormat(gl, RG16F, RG, halfFloatTexType);
        formatR = getSupportedFormat(gl, R16F, RED, halfFloatTexType);
    } else {
    ...
    // Create base shaders and programs
    ...
    function initFramebuffers() {
      ...
      if (!rg) {
    throw new Error("RG format is not supported by your browser");
     }
     if (!rgba) {
    throw new Error("RG format is not supported by your browser");
     }
    ...
    function resizeCanvas() {
      const width = scaleByPixelRatio(safeCanvas.clientWidth);
      const height = scaleByPixelRatio(safeCanvas.clientHeight);
      if (safeCanvas.width !== width || safeCanvas.height !== height) {
    ...
    function splat(
      ...
      gl.uniform1f(
        ...
        safeCanvas.width / safeCanvas.height
      );
    ...
    function correctRadius(radius: number) {
      const aspectRatio = safeCanvas.width / safeCanvas.height;
    ...
    function updatePointerDownData(pointer: Pointer, id: number, posX: number, posY: number) {
      ...
      pointer.down = true;
      pointer.moved = false;
      pointer.texcoordX = posX / safeCanvas.width;
      pointer.texcoordY = 1.0 - posY / safeCanvas.height;
